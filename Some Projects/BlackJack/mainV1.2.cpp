...
//check loose for players
void checkLose(Dealer*& dealer, int playerIndex)
{
	if(dealer->getPlayerPointer(playerIndex)->getCartsValue() > 21)
	{
		std::cout << "Player " << dealer->getPlayerPointer(playerIndex)->getPlayerName() << " lose" << std::endl;
		dealer->getPlayerPointer(playerIndex)->setKeepPlaying();
	}
}

//check for win or lose for players
void winners(Dealer*& dealer)
{
	for(int i = 0; i < dealer->getPlayerCount(); ++i)
	{
		if(dealer->getPlayerPointer(i)->getCartsValue() < dealer->getCartsValue())
		{
			std::cout << "Player#" << i + 1 << std::endl;
			std::cout << "Dealer have " << dealer->getCartsValue() << std::endl;
			std::cout << dealer->getPlayerPointer(i)->getPlayerName() << " loose" << std::endl;
			std::cout << std::endl;
		}
		else if(dealer->getPlayerPointer(i)->getCartsValue() == dealer->getCartsValue())
		{
			std::cout << "Player#" << i + 1 << std::endl;
			std::cout << "Dealer have " << dealer->getCartsValue() << std::endl;
			std::cout << dealer->getPlayerPointer(i)->getPlayerName() << " and dealer tied" << std::endl;
			std::cout << std::endl;
		}
		else
		{
			std::cout << "Player#" << i + 1 << std::endl;
			std::cout << "Dealer have " << dealer->getCartsValue() << std::endl;
			std::cout << dealer->getPlayerPointer(i)->getPlayerName() << " win" << std::endl;
			std::cout << std::endl;
		}
	}
}
...
{
	//input player's place
	std::cout << "Choose u place (1, " << countOfPlayers << std::endl;
	int playerSeat;
	std::cin >> playerSeat;
	...
	//filling playerPointers by player and bots
